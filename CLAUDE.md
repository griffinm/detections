...
**Phases 0–7 implemented** (see `plans/09-milestones.md`): the monorepo
...
(`vd_db.load_effective_settings`). All milestones are complete; consciously
deferred work is tracked in `plans/deferred.md`, operational guidance in
`docs/runbook.md`. `./plans/` remains the source of truth — read the relevant
plan before working in a subsystem and update it in the same change.
...
| `plans/deferred.md`               | Registry of consciously deferred work + tech debt     |
